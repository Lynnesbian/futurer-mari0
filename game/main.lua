...
function changescale(s, fullscreen)
	...
	if fullscreen then
		fullscreen = true
		scale = 2
		love.window.setMode(800, 600, fullscreen, vsync, msaa)
	end

	uispace = math.floor(width*16*scale/4)
	love.window.setMode(width*16*scale, 224*scale, {fullscreen = fullscreen, vsync = vsync, msaa = msaa}) --27x14 blocks (15 blocks actual height)
	...
	if keyprompt then
		for i = 1, #prompt.joysticks do
			for j = 1, #prompt.joystick[i].validhats do
				local dir = prompt.joysticks[i]:getHat(prompt.joystick[i].validhats[j])
				if dir ~= "c" then
			...
			for j = 1, prompt.joystick[i].axes do
				local value = prompt.joysticks[i]:getAxis(j)
