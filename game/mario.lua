...
function mario:update(dt)
	...
	elseif self.animation == "flag" then
		if self.animationtimer < flagdescendtime then 	
			...
			if self.y > 68/16 + flagydistance-self.height then
				...
				if self.animationtimer % flagclimbframedelay*2 >= flagclimbframedelay then
					self.climbframe = 1
				else
					self.climbframe = 2
				end
	...
	elseif self.animation == "axe" then
		...
		if bridgedisappear then
			...
			if v then
				v.walkframe = round(self.animationtimer % castleanimationbowserframedelay*2*(1/(castleanimationbowserframedelay*2)))+1
			end
	...
	elseif self.animation == "vine" then
		...
		self.climbframe = math.ceil(self.vinemovetimer % vineframedelay*2/vineframedelay)
	...
	elseif self.animation == "shrink" then
		self.animationtimer = self.animationtimer + dt
		--set frame lol
		local frame = math.ceil(self.animationtimer % growframedelay*3/shrinkframedelay)
		self:updateangle()
		...
		local invis = math.ceil(self.animationtimer % invicibleblinktime*2/invicibleblinktime)
	...
	elseif self.animation == "grow1" then
		self.animationtimer = self.animationtimer + dt
		--set frame lol
		local frame = math.ceil(self.animationtimer % growframedelay*3/growframedelay)
		self:updateangle()
	...
	if self.vine then
		...
		elseif downkey(self.playernumber) then
			...
			self.climbframe = math.ceil(self.vinemovetimer % vineframedelaydown*2/vineframedelaydown)
			self.climbframe = math.max(self.climbframe, 1)
...
function mario:updateangle()
	...
	elseif #controls[self.playernumber]["aimx"] > 0 then
		...
		--this is necessary since it's not possible to get a joystick by index anymore
...
function mario:movement(dt)
	...
	if self.falling == false and self.jumping == false then
		--bottom on grid
		if self.y+self.height % 1 == 0 then
